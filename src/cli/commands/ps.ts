...
/**
 * Pretty-print the list of running lark-channel-bridge-department processes.
 *
...
export async function runKillCli(target: string | undefined): Promise<void> {
  if (!target) {
    console.error('用法: lark-channel-bridge-department kill <bot id 或序号>');
    process.exit(1);
  }
  const entry = resolveTarget(target);
  if (!entry) {
    console.error(`✗ 没找到匹配的 bot:${target}`);
    console.error('  用 `lark-channel-bridge-department ps` 看可选目标。');
    process.exit(1);
  }
...
function findOwningService(
  ...
  for (const { serviceId, flag } of candidates) {
    ...
    return {
      ...
      stopHint: `lark-channel-bridge-department stop ${flag}`,
      restartHint: `lark-channel-bridge-department restart ${flag}`,
    };
  }
  return undefined;
}
