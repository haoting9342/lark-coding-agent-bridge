...
describe('kill on OS-managed processes', () => {
  ...
  it('refuses to SIGTERM a process the service manager owns and points at `stop`', async () => {
    ...
    expect(errors.join('\n')).toContain('lark-channel-bridge-department stop --web-ui');
    expect(errors.join('\n')).toContain('lark-channel-bridge-department restart --web-ui');
  ...
  it('names the per-profile service when that is what owns the pid', async () => {
    ...
    expect(errors.join('\n')).toContain('lark-channel-bridge-department stop --profile codex-dev');
